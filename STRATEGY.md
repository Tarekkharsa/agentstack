...
| D6 — extensions are pinned executable content | **Done — E1–E3 landed 2026-07-16 (commit 3b293c1); E4 deferred** ([design](docs/design/extensions-capability.md)) | Govern native harness extensions/plugins (pi, OpenCode first) as a first-class capability kind: strict content pinning via the D3 integrity-root digest, untrusted-means-never-rendered, copy-based render with an ownership ledger, and an honest provenance-only runtime label — the code executes inside the harness process, outside the policy ceiling, and AgentStack never executes it itself. No cross-CLI translation; the portable layer remains declarative hooks. | Witnesses: an untrusted bundle renders no extension bytes; a one-byte source edit fails locked verification and re-gates review; pruning never touches unmanaged files or the guard's artifacts. |
| D7 — workflows are governed orchestration | **Approved — W0 settled 2026-07-21; lane opened ahead of Phase 2 (maintainer scope decision).** Basis: the Phase 0A locked-run keystone (§9.1 blocker) landed 2026-07-17, and the evidence gate is restated, not waived — two real recurring tasks must run through the interim path (native orchestrator couriering into `run --locked --prompt`) before W3. Order: W2 child-run primitive → W1 pinning → W3 engine (witness- and evidence-gated) → W4 library. Supervised; does not displace the Phase 0A minimum-version cut. ([design](docs/design/workflows-capability.md)) | Govern dynamic multi-agent workflows as a capability kind with a Claude-Code-compatible authoring API: workflow source is pinned executable content (strict integrity-root digest, `roles` recorded in the pin so widening re-gates); the script executes only inside the executor domain on an embedded memory-safe interpreter — no Docker dependency and never raw on the host (maintainer ruling 2026-07-17); each `agent()` call is a governed child run whose grant is role-bound and can only narrow. Workflow files request authority; they can never grant or widen it. | W1–W3 witnesses: a one-byte script edit re-gates review; a `roles` widening without re-lock is refused as drift; an undeclared role is refused at runtime; the script cannot reach fs/net/env; ceiling exhaustion stops spawning and is recorded. |
| D8 — protection is present by default at first run | **Proposed, post-keystone** ([design](docs/design/init-access-control.md)) | `init --global` seeds a curated `[policy.filesystem]` deny default plus `[guard]`, and offers (never silently performs) `guard install` for detected CLIs. Absence-is-open semantics are unchanged — the default becomes *present*, not the semantics closed — and all first-run copy is bound to the enforcement matrix's cooperative (¶) wording. The default deny list is a reviewed security artifact. | A1/A2 witnesses: `--dry-run` shows the policy blocks and writes nothing; no overwrite without `--force`; declining the offer performs zero guard writes; the seeded default denies and audits a `.env` read through a hooked CLI; a project deny glob enforces in union with the machine list. |
| D9 — secrets are user-chosen, optional, and fail-closed | **Proposed; S1 edge fixes are in-cut-sized** ([design](docs/design/init-secrets-experience.md)) | Init offers a store choice only when real values are being lifted (OS credential store / labelled-plaintext `.env` / refs-only); an unreachable store informs and continues — it never aborts init and never silently drops values. AgentStack's write paths are exactly the keychain and project `.env`; varlock is detect-and-resolve, never authored. The resolution chain and rule 5 (fail-closed, `${REF}`-only serialization) are untouched. | S1–S3 witnesses: init with a failing store completes and reports unstored refs by name; the `.env` path never clobbers existing keys and verifies gitignore; `secret lift` moves values without altering the manifest and a subsequent `apply` resolves identically. |
...
| Post-keystone, pre-distribution | D8 first-run protection defaults + D9 secrets onboarding (S1 edge fixes in-cut-sized) | Converts shipped enforcement and fail-closed secret resolution into visible first-run value; both are UX over existing machinery, no new semantics |
...
| P2 after runner evidence | Saved governed workflows with role-specific grants (design banked 2026-07-17: D7) | Makes proven tasks reusable without widening authority |
