...
1. **`#![forbid(unsafe_code)]`** at the top of every crate — with one shipped exception: the `cli` crate retains a handful of libc process-management call sites (`kill`/`dup` in `runs.rs`, `gateway.rs`, `mcp_server.rs`); wrap or replace them before Phase 2. Never write new `unsafe` anywhere, and every *extracted* crate carries the forbid from day one.
