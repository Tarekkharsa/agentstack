...
pub fn add_skill(
    ...
    let (entry, dest, checksum, source_kind, total_bytes) = match source {
        LibSource::Path(src) => {
            ...
            (entry, Some(dest), checksum, "path", total_bytes)
        }
        LibSource::Git { url, rev } => {
            ...
            (entry, None, resolved.checksum, "git", total_bytes)
        }
    };
...
fn add(args: &LibAddArgs) -> Result<()> {
    ...
    if outcome.total_bytes > LARGE_SKILL_BYTES {
        println!(
            "{} '{}' is {} — every full-library pass (doctor, install, use) reads all of it. \
