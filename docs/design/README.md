...
| [`packaging.md`](packaging.md) | What an image is — one toolset's pinned members composed into a self-run Docker image, how it reuses the sandbox/egress contract instead of duplicating it, the posture label it carries and what that label refuses to promise, why secrets are required at run time and never baked, and exactly which reproducibility is and is not claimed | Active contract; adopted 2026-08-03 |
