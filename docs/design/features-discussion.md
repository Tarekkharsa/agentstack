...
**SHIPPED (2026-07-19):** The consent moment must teach; a re-trust that formats a new server identically to reviewed ones is the consent-fatigue trap this product exists to avoid. Display-only change; trust granting logic untouched (security-review flag anyway).
...
**SHIPPED (2026-07-19):** always name the machine ceiling file at consent, one line. Pure "explain the why at the moment" (principle 4, P11 sibling).
...
**SHIPPED (2026-07-19):** bare orientation explains inert-until-trusted in one line and makes `trust .` the next step. Doorway principle (P29 sibling).
...
**SHIPPED (2026-07-19):** the dry-run is the decision moment; the trust-unlock line moves there. P1 discipline (say it before, not after).
...
**SHIPPED (2026-07-19):** as a FOLD, not a new verb (P27 instinct: no new front doors): bare orientation names profiles + marks the active one; the multi-profile disambiguation lists each with server/skill counts. No `agentstack profiles` command.
...
**SHIPPED (2026-07-19):** the error names the library skill and shows the exact fix; activation warns when an inline block shadows a library name. Teach-at-error is the house signature (P3 sibling).
...
**SHIPPED (2026-07-19):** "source gone — library copy canonical" instead of a dead path. Truthful output (P6 sibling).
...
**SHIPPED (2026-07-19):** one contrast paragraph in the reference + one line in `explain`. The P19 trap grows from this gap.
...
**SHIPPED (2026-07-19):** `--plan` enumerates every gate exactly as the live run would. The README literally promises "prints every gate decision"; make it true (machine transparency).
...
**SHIPPED (2026-07-19):** the trust-drift refusal also teaches the re-lock half when the lock is stale (completes P9; the rule must be taught wherever it bites).
...
**SHIPPED (2026-07-19):** prerequisite check before the banner; never paint a posture you cannot deliver (P6 truthfulness).
