...
| Concrete fixes from the signals already collected | `agentstack x optimize` |
| One server or skill, **before** you trust or rely on it | `agentstack x explain <name>` |

```bash
# What ran, and what one run did
agentstack x report runs                # table of tracked runs (--json to script)
agentstack x report run <id>            # one run's flight recorder + posture label
agentstack x kill <id>                  # stop a tracked run that's gone wrong

# What your capabilities cost and call
agentstack x report usage               # per-capability context cost + activation counts
agentstack x report usage --live        # measure each server's tools/list on the wire
agentstack x report calls --since 7     # brokered tool calls, last 7 days (--json to script)
agentstack x report wire                # per-turn token weight of the tools block

# Turn evidence into next actions
agentstack x optimize                   # evidence-backed recommendations (--write applies the safe class only)

# Before you trust or rely on a capability
agentstack x explain <name>             # provenance, effective policy, and context cost, for one capability
```

**Start here:** `agentstack x report runs`. If it is empty, nothing has been
