...
you are about to change and linking to the section that teaches it.
...
machine-wide state. Later steps cannot deliver anything until the bridge is
registered.
...
records the folder's path. It copies no files, and any folder can be a library.
...
no duplicated tokens. The repo carries two files: the manifest and the lock.
...
and verifies the checkout against its lockfile, because the lock pins exactly
what the first machine reviewed.
