...
3. **Connect** — `agentstack more gateway connect --all --write`: the live lane
   needs one bridge registered per MCP-capable CLI.
4. **Route** — `agentstack more delivery`: both CLIs are MCP-capable, so the servers
...
6. **Render anyway** — `agentstack more delivery render-locally --write`, then
...
7. **Undo** — `agentstack more restore --last --write`, four times (the render, the
