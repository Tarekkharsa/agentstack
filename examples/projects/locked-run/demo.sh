...
scene_safe() {
  ...
  run "$AS lock --write && $AS trust ." "$AS lock --write >/dev/null && $AS trust . --yes --consented-digest \"\$($AS trust . --preview | sed -n 's/.*\"surface_digest\": \"\([^\"]*\)\".*/\1/p')\" 2>&1 | sed -n '1,3p'"
...
EOF
  ...
  run "$AS lock --write && $AS trust ." "$AS lock --write >/dev/null && $AS trust . --yes --consented-digest \"\$($AS trust . --preview | sed -n 's/.*\"surface_digest\": \"\([^\"]*\)\".*/\1/p')\" >/dev/null 2>&1; echo 'pinned + trusted'"
...
scene_drift() {
  ...
  run "$AS run claude-code --locked" "$AS lock --write >/dev/null && $AS trust . --yes --consented-digest \"\$($AS trust . --preview | sed -n 's/.*\"surface_digest\": \"\([^\"]*\)\".*/\1/p')\" >/dev/null 2>&1; $AS run claude-code --locked 2>&1 | tail -n 2"
  ...
  run "$AS lock --write && $AS trust . && $AS run claude-code --locked" "$AS lock --write >/dev/null && $AS trust . --yes --consented-digest \"\$($AS trust . --preview | sed -n 's/.*\"surface_digest\": \"\([^\"]*\)\".*/\1/p')\" >/dev/null 2>&1; $AS run claude-code --locked 2>&1 | tail -n 2"
  teardown
}
