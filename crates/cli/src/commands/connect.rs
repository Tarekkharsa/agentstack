//! `agentstack more gateway connect` / `disconnect` — register the zero-files gateway.
//!
    ...
    if !ids.is_empty() {
        ...
        for id in ids {
            let desc = registry.get(id).with_context(|| {
                format!("unknown adapter '{id}' (see `agentstack more adapters list`)")
            })?;
...
pub const GATEWAY_RECOVERY: &str = "agentstack more gateway connect --all";
