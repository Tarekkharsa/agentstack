//! `agentstack more uninstall` — take everything AgentStack put on this machine
...
fn machine_state_only(args: &UninstallArgs, no_manifest: anyhow::Error) -> Result<()> {
    ...
    if !args.write {
        ...
        println!(
            "{}",
            "Rendered native config in a project is removed by running `agentstack more uninstall` \
    ...
    println!(
        "  {}",
        "Any rendered config still in a project is untouched — run `agentstack more uninstall` \
