...
fn files_to_scan(root: &Path) -> Vec<(PathBuf, Kind)> {
    ...
    // docs/ itself, plus the reader-facing subdirectories. `howto/` and
    // `tutorial/` are entry points a person actually reads, and both are
    // generated from Markdown, so they are covered by the doc tooling like
    // every other page. (`panel/` used to be scanned here because
    // `panel/index.html` was hand-authored with no Markdown source and this
    // was the only gate that ever read its command names; the page was
    // retired — it simulated a second UI — and `docs/panel/index.html` is now
    // a redirect stub the loop below skips.) Skipped on purpose:
...
fn every_dynamic_command_parses() {
    ...
    // trips before the gate quietly stops guarding anything. It was 30 while
    // `docs/cookbook.html` was a source; that page carried ~20 of the copyable
    // commands on the site and was retired (unverified hand-typed transcripts),
    // so the floor is re-baselined to what the four remaining sources actually
    // hold. Lower it again only alongside a page that genuinely goes away.
    assert!(
        checked >= 10,
